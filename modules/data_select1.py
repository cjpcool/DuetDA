...
class DuetDALoader(Dataset):
    ...
    def __init__(
        ...
        self.score_gen = np.ones(n, dtype=np.float32) * 3 if scores is None else scores.astype(np.float32)
        self.select_sep = True if (self.method == "duetda")  else False
        self.sel_phy_rat = 0.5  # record selected physics ratio
    ...
    def valuator_forward_only(self, batch_feat: torch.Tensor | None = None, weight_phy=None, weight_gen=None, status=None, **kwargs) -> torch.Tensor:
        ...
        return  self.valuator(batch_feat, weight_phy=weight_phy, weight_gen=weight_gen, status=status, **kwargs)
    ...
    def update(
        ...
        **kwargs
        ...
        if self.method == "duetda":
            ...
            if not self.select_sep:
                s = self.valuator_forward_only(batch_feat, weight_phy=weight_phy, weight_gen=weight_gen, status=status, ret_sep=False)
            else:
                val_phy, val_gen, weight_phy_out, weight_gen_out = self.valuator_forward_only(batch_feat, weight_phy=weight_phy, weight_gen=weight_gen, status=status, ret_sep=True)
                if self.ablation == "wo_gate":
                    s = val_phy + val_gen
                elif self.ablation == "wo_phy":
                    s = val_gen
                elif self.ablation == "wo_gen":
                    s = val_phy
                else:  # full
                    s = val_phy * weight_phy_out + val_gen * weight_gen_out
            s = s.view(-1)
        else:
    ...
    def prune(self, seed: int):
        ...
        if self.all_losses is None:
            self.all_losses = np.ones(n, dtype=np.float32) * 3  # initialize all_losses if not yet
