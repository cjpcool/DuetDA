...
#             data, indices, w = self._next_data()
#         else:
...
def info_hack_indices(self):
    with torch.autograd.profiler.record_function(self._profile_name):
        ...
        data, indices, w = batch, None, None
        if isinstance(batch, (tuple, list)):
            if len(batch) == 3:
                data, indices, w = batch
            elif len(batch) == 2:
                data, indices = batch
        ...
        if self._dataset_kind == _DatasetKind.Iterable and \
           self._IterableDataset_len_called is not None and \
           self._num_yielded > self._IterableDataset_len_called:
            warnings.warn("IterableDataset length mismatch ...")

        if isinstance(self._dataset, InfoBatch) and indices is not None:
            self._dataset.set_active_indices(indices)

        return (data, indices, w) if indices is not None else data
...
class InfoBatch(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # adapt to cgcnn
        return self.dataset[index], int(index), torch.tensor(1.0)
    ...
    def prune(self):
        ...
        remained_indices = remained_indices[:int(self.keep_ratio * len(self.dataset))]
...
class SeTa(InfoBatch):
    ...
    def prune(self):
        ...
        ###### ensure each epoch prune same sample numbers
        k = int(self.keep_ratio * len(self.dataset))
